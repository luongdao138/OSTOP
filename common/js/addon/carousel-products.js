...
----------------*/
jQuery(function($){
	$.fn.carouselProducts = function(options) {
		if ($(this).size() == 0) return false;
		var self = this;
		var opts = $.extend($.fn.carouselProducts.defaults,options);
		var CarouselCore = {
			targetName : self,
			wrapperWidth : 0,
			wrapperHeight : 0,
		...
		var Carousel = {
			...
			viewSelectImg : function(indexNum) {
				$('.productsImg > p').hide();
				$('.productsImg > p').eq(indexNum).show();
			},
		...
		$('.carouselItem', CarouselCore.targetName).bind('click', function(e) {
			var indexNum = $('.carouselItem', CarouselCore.targetName).index(this);
			Carousel.viewSelectImg(indexNum);
		});
...
jQuery(function($){
	$('.productsInfo .carouselItem img').lazyload({
		placeholder : '/common/images/carousel/products-loading.gif',
		effect : 'fadeIn',
		event : 'customScrollEvent'
	});

	$('#carouselHrProducts-1').carouselProducts();
	/* $('.carouselHrProducts').each(function(i) {
		var targetName = '#carouselHrProducts-' + (i + 1);
		$(targetName).carouselProducts();
	}); */
});
