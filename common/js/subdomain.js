...
(function($, window, document, undefined){
	...
	// 会員管理用
	window.DOMAIN_MEM = (document.domain === 'www.starbucks.co.jp'
		|| document.domain === 'starbucks.co.jp'
		|| document.domain === 'login.starbucks.co.jp'
		|| document.domain === 'member.starbucks.co.jp'
		|| document.domain === 'store.starbucks.co.jp'
		...
		|| document.domain === 'gac.starbucks.co.jp'
		|| document.domain === 'sbgc.starbucks.co.jp')
		? '//member.starbucks.co.jp'
		: '//member2.starbucks.co.jp';

	// CloudFront用
	window.DOMAIN_CF = (document.domain === 'www.starbucks.co.jp'
		|| document.domain === 'starbucks.co.jp'
		|| document.domain === 'login.starbucks.co.jp'
		|| document.domain === 'member.starbucks.co.jp'
		|| document.domain === 'store.starbucks.co.jp'
...
// 2度押し防止
$(function(){
	$('.js-preventDoubleClick').click(function(){
		// バリデーションエラーが発生していないか判定
		var $form = $(this).parents('form');
		var isValidateError = ($form.find('.is-error').length)? true : false ;
		//console.log($form.find('.is-error').length+', '+isValidateError);
		
		// is-doubleClick状態ではない、かつバリデーションエラーが発生していないときのみ、リンクさせる
		if((!$(this).hasClass('is-doubleClick'))&&(!isValidateError)){
			...
			// 要素に応じて処理を変更
			if($(this).attr('href')){
				console.log("リンク遷移しました");
				location.href = $(this).attr('href');
			}else if($(this).attr('type')==='submit'){
				console.log("フォーム送信しました");
				$form.submit();
			}
		}
		return false;
...
$(function(){
    ...
    /*
	$.ajax({
		url: window.DOMAIN_WWW_API+'/api/getNickName.php',
		timeout: 30000,
		dataType: 'jsonp',
...
$(function (){
	...
	if(orientation != undefined){
		...
		// デフォルト値
		var defaultViewPort = $('meta[name=viewport]').attr('content');
	...
	function setViewport(){
		...
		// 比率判定
		if(device == 'sp'){
			var scale = width/colW;
			var deviceWidth = colW;
		}else if(device == 'tab'){
			var scale = width/colW/2;
			var deviceWidth = colW*2;
		}else if(device == 'pc'){
			var scale = width/colW/3;
			var deviceWidth = colW*3;
		}
		...
		/*
			...
			minimum-scale：最大倍率
			maximum-scale：最大倍率
			user-scalable：ズームの操作（yes/no）
		*/
		/*
		var viewPort= 'width='+deviceWidth+', initial-scale='+scale+', minimum-scale='+scale+', maximum-scale='+scale+', user-scalable=no';
	...
	if ((navigator.userAgent.indexOf('Android') > 0 && navigator.userAgent.indexOf('Mobile') > 0)){
		$(window).bind('resize load', function(){
			...
			$("body.forSubdomain .overlay").css("zoom" , 320/$(window).width() );
		});
	}
});
*/
...
$(function (){
	...
	function setMegaNavSlide(obj){
		...
		// 幅などの初期値
		var newsContainerItemWidth = newsContainer.find('.news').innerWidth(); // 表示エリアサイズ
		var newsItem = newsContainer.find('.newsSnap li');
		var newsItemNum = newsItem.size(); // アイテム数
		var newsContainerWidth = newsContainerItemWidth*newsItemNum; // 表示エリアサイズ x アイテム数
		var distance = newsContainerItemWidth; // 一度の移動幅
		...
		if(defaultCol[ovClass] == 2){
			// 2カラムの場合
			var maxPoint = Math.ceil((newsItemNum-2) / 2); // インジケータ用（2を引くのは2アイテム分だから）
			var maxPoint2 = Math.ceil((newsItemNum) / 2)+1; // インジケータCSS用（指定数以降が消えるので1プラス）
			newsContainerItemWidth = newsContainerItemWidth / 2; // 表示幅が2倍なので、1/2にする
		}else{
			// 1カラムの場合
			var maxPoint = (newsItemNum-1); // インジケータ用
			var maxPoint2 = (newsItemNum+1); // インジケータCSS用
		}
		...
		if(defaultPoint[ovClass] != undefined){
			...
			// チェンジ無しの場合でも位置を調整する必要があるのでif文の外
...
$(function (){
	...
	$('.js-shareFb,.js-shareTw,.js-shareLn').on('click',function(){
		...
		// PC
			...
			if(sys == 'js-shareFb'){
				...
				window.open('https://twitter.com/intent/tweet?url='+url+'&amp;text='+text,'share','width=632,height=456,location=yes,resizable=yes,toolbar=no,menubar=no,scrollbars=no,status=no');
			}
		}
		return false;
	});
	...
	$('.js-pulldown').on('click',function(){
		...
		if(obj.is(':visible')){
			...
			$('.refine.js-component').removeClass('is-opened');
...
$(function (){
	$(window).on("load", function(){
		if(document.URL.match('/store/search/result.php')) {
			var winH = $(window).height();
			var footerH = $('footer.global').outerHeight();
			$('body').addClass('isFullScreen');
			
			$('div.mainContents.store.searchResult').css('min-height', winH-footerH);
			$('div.mainContents.store.searchResult .store').css('min-height', winH-footerH);
			$(window).resize(function(){
				var winH = $(window).height();
				var footerH = $('footer.global').outerHeight();
...
$(function(){
	...
	var cutFigureCSR = '35'; // カットする文字数（CSR）
	var cutFigurePR = '37'; // カットする文字数（プレスリリース）
...
function setUtilityNavWithFooter(){
	...
	if(0 < $("nav.utility.os, nav.utility.is-cartActive").size()){
		...
		//$('nav.utility js-expandableContent')
	}
}
...
// カラム数制御
/*
function setColClear(){
	$('.row').each(function(){
		var rowW = $(this).width();
		var col = $(this).find('.col1');
		col.removeClass('clear');
		var colW = col.innerWidth();
		// tabの2カラムを考慮して四捨五入
		var colNum = Math.round(rowW/colW*100);
		colNum = colNum/100;
		var i = 0;
		$(this).find('.col1').each(function(){
			if(i%colNum == 0) $(this).addClass('clear')
			i++;
		});
	});
}*/
