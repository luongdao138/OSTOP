// create carousel
let carousels = document.getElementsByClassName('store-information carousel');
...
let carouselWraps = document.getElementsByClassName(
  'store-information carousel-wrap'
);
...
function animateCarousel() {
  ...
  for (let carouselWrap of carouselWraps) {
    ...
    let carouselItemLastMarginRight = parseInt(
      window.getComputedStyle(carouselItemLast, null).marginRight
    );
    let carouselItemWidthWholeLast =
      carouselItemWidthLast + carouselItemLastMarginRight;
    ...
    nextButton.addEventListener('click', function () {
      ...
      for (
        let i =
          activeCarouselItemNum * currentPage -
          (restItems > activeCarouselItemNum
            ? 0
            : activeCarouselItemNum - restItems);
        i < targetIndexMax;
        i++
      ) {
      ...
      if (restItems > activeCarouselItemNum || currentPage + 1 !== maxPage) {
        ...
        const paddingLeftCarouselInner =
          parseFloat(window.getComputedStyle(carouselInner).paddingLeft) || 0;
        const restWidth =
          carouselWrapWidth -
          (activeCarouselItemNum * carouselItemWidthWhole +
            paddingLeftCarouselInner) +
          (carouselItemWidthWhole - carouselItemWidthWholeLast);

        position +=
          carouselItemWidthWhole * restItems -
          restWidth +
          paddingLeftCarouselInner;
      }
